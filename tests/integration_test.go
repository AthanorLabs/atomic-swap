...
func (s *IntegrationTestSuite) TestSuccess_OneSwap() {
	s.testSuccessOneSwap(types.EthAssetETH, false)
}

func (s *IntegrationTestSuite) testSuccessOneSwap(asset types.EthAsset, useRelayer bool) {
	const testTimeout = time.Second * 90

	ctx, cancel := context.WithTimeout(context.Background(), testTimeout)
	defer cancel()

	bwsc := s.newSwapdWSClient(ctx, defaultXMRMakerSwapdPort)
	...
	ac := rpcclient.NewClient(ctx, defaultXMRTakerSwapdPort)
	awsc := s.newSwapdWSClient(ctx, defaultXMRTakerSwapdPort)
...
func (s *IntegrationTestSuite) testAbortXMRMakerCancels(asset types.EthAsset) {
	...
	c := rpcclient.NewClient(ctx, defaultXMRTakerSwapdPort)
	wsc := s.newSwapdWSClient(ctx, defaultXMRTakerSwapdPort)
...
func (s *IntegrationTestSuite) testErrorShouldOnlyTakeOfferOnce(asset types.EthAsset) {
	...
	go func() {
		defer wg.Done()
		wsc := s.newSwapdWSClient(ctx, defaultCharlieSwapdPort)
